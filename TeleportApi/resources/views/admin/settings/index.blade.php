    ...
    <form action="{{ route('admin.settings.store') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="block">
            ...
            <div class="block-content">
                ...
                <h2 class="content-heading">Настройка текста на узбекском языке (латиница)</h2>
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label for="faq_latuz">Вопросы и ответы</label>
                            <textarea name="faq_latuz" id="faq_latuz" class="form-control">{!! $settings->faq_latuz !!}</textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label for="about_latuz">О нас</label>
                            <textarea name="about_latuz" id="about_latuz" class="form-control">{!! $settings->about_latuz !!}</textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label for="partners_latuz">Наши партнёры (Реклама)</label>
                            <textarea name="partners_latuz" id="partners_latuz" class="form-control">{!! $settings->partners_latuz !!}</textarea>
                        </div>
                    </div>
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label for="partners_ad_image_latuz">Изображение для рекламы</label>
                            <input type="file" name="partners_ad_image_latuz" id="partners_ad_image_latuz">
                            @if ($settings->partners_ad_image_latuz)
                                <a href="{{ route('admin.settings.delete', 'latuz') }}" class="btn btn-alt-danger"><i class="fa fa-trash"></i> Удалить</a>
                            @endif
                        </div>
                        @if ($settings->partners_ad_image_latuz)
                            <img src="{{ $settings->getAdImageLatUz() }}" alt="" class="w-50">
                        @endif
                    </div>
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label for="partners_tariffs_latuz">Наши партнёры (Тарифы)</label>
                            <textarea name="partners_tariffs_latuz" id="partners_tariffs_latuz" class="form-control">{!! $settings->partners_tariffs_latuz !!}</textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label for="news_latuz">Новости</label>
                            <textarea name="news_latuz" id="news_latuz" class="form-control">{!! $settings->news_latuz !!}</textarea>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <label for="support_latuz">Поддержка</label>
                            <textarea name="support_latuz" id="support_latuz" class="form-control">{!! $settings->support_latuz !!}</textarea>
                        </div>
                    </div>
                    <div class="col-sm-12">
                        <div class="form-group">
                            <div class="form-material">
                                <label for="support_image_latuz">Изображение раздела поддержки</label>
                                <input type="file" name="support_image_latuz" id="support_image_latuz" class="form-control">
                            </div>
                        </div>
                        @if ($settings->support_image_uz)
                            <img src="{{ asset('storage/' . $settings->getSupportImageLatUz()) }}" alt="" class="img-thumbnail w-50">
                        @endif
                    </div>
                </div>
            </div>
