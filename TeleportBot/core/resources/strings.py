...
# Uzbek language kirill symbols
_strings_uz = json.loads(open(os.path.join(_basedir, 'strings_uz.json'), 'r', encoding='utf8').read())

# Uzbek language latin symbols
_strings_lat_uz = json.loads(open(os.path.join(_basedir, 'strings_lat_uz.json'), 'r', encoding='utf8').read())
...
def get_string(key, language='ru') -> str:
    if language == 'ru':
        ...
        return _strings_lat_uz.get(key, 'no_string')
    else:
